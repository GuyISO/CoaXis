...
### 1.5 開発方針
- 主技術は .NET 8.0 / C#
- CATIAはVBAを併用（既存資産流用、Excel/CATIA連携、移行リスク低減）
- BlenderはPythonを併用（CoaXisModelConverterによる3Dモデル最適化およびGLB変換）
...
7. PublisherがMES登録用データを出力する

### 3.4 Planner-Viewer責務分離
- Planner: データ操作、マスタ管理、業務UI、API通信
...
### 4.1 CoaXisExtractor
目的:
- CATIA V5 MBDやResourceモデルから業務情報を抽出し、CoaXisの入力データを生成する

責務:
- フォルダ単位抽出（Document単位）
- Product/Instance/Component抽出
- Point/Line/Surface/Annotation/Property抽出
- wrl出力
...
出力:
- coaextファイル（以下を含むパッケージ）
- DocumentMaster登録csvデータ
- wrlファイル
...
責務:
- 3D中間データの読み込みとGLB変換
- 視覚用モデルおよび衝突判定用モデルの生成
- メッシュ統合、不要データ削除、座標系およびスケールの整合
- 変換条件・入力・出力を記録した処理ログの出力

入力:
- CoaXisExtractorが出力した3D中間データ
- 変換設定（品質、最適化、視覚用/衝突用の生成条件など）
...
制約:
- 業務データおよびDBへ直接アクセスしない
- Blender固有の編集・変換処理をSceneBuilderやViewerへ持ち込まない
- 変換失敗時は入力単位で再実行できるよう、終了ステータスとログを出力する
