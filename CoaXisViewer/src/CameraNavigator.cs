...
public partial class CameraNavigator : Control
{
	...
	private const float TrackballCrossAngularSize = 0.12f;
	...
	private void OnCameraControlModeChanged(CameraController.Mode mode)
	{
		...
		{
			CaptureTrackballAnchor(GetViewport().GetMousePosition(), mode);
		}
	...
	private void CaptureTrackballAnchor(Vector2 mousePos, CameraController.Mode mode)
	{
		if (_cameraController == null || _cameraController.Camera == null)
		{
			ClearTrackballCross();
		...
		Vector2 anchorMousePos = mode == CameraController.Mode.Roll
			? ClampMouseToTrackballBoundary(mousePos)
			: mousePos;

		Vector3 anchorCamera = ProjectOntoTrackball(anchorMousePos);
	...
	private Vector2 ClampMouseToTrackballBoundary(Vector2 mousePos)
	{
		Vector2 fromCenter = mousePos - _cameraController.ScreenCenter;
		float radius = _cameraController.TrackballRadius;

		if (fromCenter.LengthSquared() < 1e-6f)
		{
			return _cameraController.ScreenCenter + Vector2.Right * radius;
		}

		return _cameraController.ScreenCenter + fromCenter.Normalized() * radius;
	}
