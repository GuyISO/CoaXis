...
/// <summary>
/// CATIA における航空機の座標系 (Z+ Up, X+ Back, Y+ Right) を
/// Godot 標準の座標系 (X+ Right, Y+ Up, Z+ Back) へ相互変換するユーティリティ。
/// </summary>
/// <remarks>
/// 運用ルール:
...
/// - 軸の対応は Xc -> Zg、Yc -> Xg、Zc -> Yg とする。
/// </remarks>
public static class CoordinateSystemUtility
{
    private const float MillimetersPerMeter = 1000.0f;
    private static readonly Basis CatiaToGodotCoordinateBasis = new Basis(Vector3.Back, Vector3.Right, Vector3.Up);
    private static readonly Basis GodotToCatiaCoordinateBasis = new Basis(Vector3.Up, Vector3.Back, Vector3.Right);
    ...
    {
        // CATIA の正方向を Godot の正方向へ対応付け、単位は保持する。
        // Xg = Yc, Yg = Zc, Zg = Xc
    ...
    {
        // Godot の正方向を CATIA の正方向へ対応付け、単位は保持する。
        // Xc = Zg, Yc = Xg, Zc = Yg
    ...
    {
        // 姿勢は座標表現そのものを変えるため、各軸だけでなく基底の入出力を共役変換する。
        return (CatiaToGodotCoordinateBasis * catiaBasis * GodotToCatiaCoordinateBasis).Orthonormalized();
    }
    ...
    {
        // 逆方向も同じ基底変換を反転して適用し、CATIA と Godot の往復で姿勢を保つ。
        return (GodotToCatiaCoordinateBasis * godotBasis * CatiaToGodotCoordinateBasis).Orthonormalized();
    }
