...
public static class ModelLoadUtility
{
    ...
    {
        // 所要時間計測開始
        Application.Instance.System.Log.Info($"Start loading model: {path}");
        ...
        if (error == Error.Ok)
        {
            ...
            sw.Stop();
            Application.Instance.System.Log.Info($"Finished loading model: {path} in {sw.ElapsedMilliseconds} ms");
            return true;
        }
        else
        {
            Application.Instance.System.Log.Error($"Failed to load model: {path}, Error: {error}");
            return false;
        }
    }
