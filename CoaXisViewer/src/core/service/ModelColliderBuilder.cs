...
public static class ModelColliderBuilder
{
    ...
    {
        Application.Logger.Info($"Start adding collider for model: {model.Name}");
        ...
        if (index == 0)
        {
            Application.Logger.Warn($"Skipped adding collider for model: {model.Name}, no mesh faces found.");
            return;
        }
        ...
        stopwatch.Stop();
        Application.Logger.Info($"Finished adding collider for model: {model.Name} in {stopwatch.ElapsedMilliseconds} ms");
    }
