...
public static class ModelColliderBuilder
{
    ...
    {
        Application.System.Log.Info($"Start adding collider for model: {model.Name}");
        ...
        if (index == 0)
        {
            Application.System.Log.Warn($"Skipped adding collider for model: {model.Name}, no mesh faces found.");
            return;
        }
        ...
        stopwatch.Stop();
        Application.System.Log.Info($"Finished adding collider for model: {model.Name} in {stopwatch.ElapsedMilliseconds} ms");
    }
